...
def launch_setup(context):
    ...
    if use_namespace == 'false':
        # ekf_param = ReplaceString(source_file=os.path.join(controller_package_path, 'config/ekf_analog_odom.yaml'), replacements={'namespace/': ''})
        ekf_param = ReplaceString(source_file=os.path.join(controller_package_path, 'config/ekf.yaml'), replacements={'namespace/': ''})
    else:
        # ekf_param = ReplaceString(source_file=os.path.join(controller_package_path, 'config/ekf_analog_odom.yaml'), replacements={"namespace/": (namespace, '/')})

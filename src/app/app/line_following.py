...
class LineFollowingNode(Node):
    def __init__(self, name):
        ...
        self.left_min_publisher = self.create_publisher(Float32, '~/min_distance_left', 10)
        self.right_min_publisher = self.create_publisher(Float32, '~/min_distance_right', 10)
    ...
    def lidar_callback(self, lidar_data):
        ...
        if len(min_dist_left_) > 0 and len(min_dist_right_) > 0:
            ...
            left_msg = Float32()
            left_msg.data = float(min_dist_left)
            self.left_min_publisher.publish(left_msg)
            right_msg = Float32()
            right_msg.data = float(min_dist_right)
            self.right_min_publisher.publish(right_msg)

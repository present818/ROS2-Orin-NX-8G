...
class NavigationTransport(Node):
    ...
    def __init__(self, name):
        ...
        self.find_client = self.create_client(Trigger, '/automatic_pick/find', callback_group=timer_cb_group)
        ...
        self.find_client.wait_for_service()
    ...
    def goal_callback(self, msg):
        ...
        self.get_logger().info('\033[1;32m%s\033[0m' % 'status    '+str(status))
        if status == 'start' : # 处于可以寻找目标的状态
            self.find = True
            self.pick = False
            self.place = False
            self.get_logger().info('\033[1;32m%s\033[0m' % 'find1111111 the target')
            self.navigator.goToPose(msg)
            self.haved_publish_goal = True

        if status == 'get_target':  # 处于可以pick的状态(in the state of ready to pick)
            self.find = False
            self.pick = True
            self.place = False
            self.get_logger().info('\033[1;32m%s\033[0m' % 'nav pick1111111')
        ...
        elif status == 'place_finish':  # 处于可以place的状态(in the state of ready to place)
            self.find = False
            self.pick = False
            self.place = False
            self.get_logger().info('\033[1;32m%s\033[0m' % 'nav place')
        ...
        if self.haved_publish_goal:
            ...
            if result == TaskResult.SUCCEEDED:
                ...
                if self.find:
                    res = self.send_request(self.find_client, Trigger.Request())
                    if res.success:
                        self.get_logger().info('start find the target')
                    else:
                        self.get_logger().info('start find the target failed')

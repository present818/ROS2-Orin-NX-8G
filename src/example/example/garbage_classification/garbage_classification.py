...
class GarbageClassificationNode(Node):
    ...
    def init_process(self):
        ...
        self.agc.run_action('garbage_pick_init')
        if self.debug:
            self.pick_roi = [30, 450, 30, 610]
            self.agc.run_action('garbage_pick_debug')
            time.sleep(5)
            self.agc.run_action('garbage_pick_init')
            time.sleep(2)
    ...
    def pick(self):
        while self.running:
            ...
            if self.start_pick:
                ...
                self.agc.run_action('garbage_pick')
                if waste_category == 'food_waste':
                    self.agc.run_action('place_food_waste')
                elif waste_category == 'hazardous_waste':
                    self.agc.run_action('place_hazardous_waste')
                elif waste_category == 'recyclable_waste':
                    self.agc.run_action('place_recyclable_waste')
                elif waste_category == 'residual_waste':
                    self.agc.run_action('place_residual_waste')
                self.agc.run_action('garbage_pick_init')
                time.sleep(0.5)
                self.start_pick = False
                self.start_srv_callback(Trigger.Request(), Trigger.Response())

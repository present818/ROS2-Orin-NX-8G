...
global_camera_type = os.environ.get('DEPTH_CAMERA_TYPE', 'default')
display_size = [int(640), int(400)] if global_camera_type == 'aurora' else [int(640), int(480)]
...
## 严格约束(Important constraints)
- 不允许以“仅包含工具调用”的助手消息作为本轮对话的最后一条回复。
- 即使工具的返回结果已经很清晰，你也必须再用自然语言向用户确认执行结果”
...
position_dict = {
    "书房": [1.2, -0.8, 0.0, 0.0, 80.0],
    "卧室": [1.38, 0.50, 0.0, 0.0, 10.0],
    "水果超市": [3.02, -0.49, 0.0, 0.0, -90.0],
    "厨房": [0.10, 0.25, 0.0, 0.0, 0.0],
    "3号分拣台": [1.15, 0.1, 0.0, 0.0, 0.0],
}
...
class LLMControlMove(Node):
    ...
    def setup_subs_and_pubs(self):
        ...
        # 巡线相关(Line following related)
        self.create_subscription(Float32, 'line_following/min_distance_left', self.left_callback, 10)
        self.create_subscription(Float32, 'line_following/min_distance_right', self.right_callback, 10)
    ...
    def move_to_location(self, destination):
        ...
        if destination not in position_dict:
            return f"移动失败：未知的目标地点 '{destination}'。"
        ...
        if self.reach_goal:
            return f"已成功抵达{destination}"
        else:
            return f"移动超时，未能到达{destination}"
    ...
    def find_nearest_location(self, current_x, current_y, position_dict):
        ...
        if nearest_location_name:
            if min_distance < 0.2:
                return f'我现在在{nearest_location_name}'
            else:
                return f'我现在在{nearest_location_name}附近'
        else:
            return '我现在暂时不知道在哪里'
    ...
    def describe_current_view(self, question):
        ...
        try:
            ...
            return f"画面描述任务已成功执行。得到的结果是{description}"
    ...
    def line_following(self, color):
        ...
        return f"好的，马上开始沿着{color}线行驶。"

    def line_lidar_scan_detect(self, scan_detect):
        """激光雷达障碍物检测(LiDAR obstacle detection)"""
        self.get_logger().info('\033[1;32m%s\033[0m' % "start line_lidar_scan_detect")
        time.sleep(2)

        while self.line_following_start:
            if (self.current_left is not None and self.current_left < 0.4) or \
               (self.current_right is not None and self.current_right < 0.4):

                if self.line_following_count < 200:
                    self.line_following_count += 1
                    if self.line_following_count % 10 == 0:
                        self.get_logger().info('\033[1;32m%s\033[0m' % "Detected obstacle")
                else:
                    self.line_follower_exit_client.call_async(Trigger.Request()) 
                    self.line_following_count = 0
                    time.sleep(2)
                    return '检测到前方有障碍物,已停止巡线'
            else:
                self.line_following_count = 0
            time.sleep(0.01)
        ...
        while rclpy.ok():
            if self.tools:
                ...
                try:
                    ...
                    elif tool_name == 'line_lidar_scan_detect':
                        scan_detect = args_dict.get('scan_detect')
                        if scan_detect:
                            res = self.line_lidar_scan_detect(scan_detect)
